...
Item { // Window
    ...
    //     // 1. Determine the 'raw' name from existing data
    //     let iconName = entry?.icon ?? windowData?.class ?? "application-x-executable";
        
    //     // 2. Apply our "Fixes" logic
    //     // If it's a Steam app ID, point it to the generic steam icon
    //     if (iconName.startsWith("steam_app_")) {
    //         iconName = "steam";
    //     }
        
    //     // If the class is empty but we have a title (common in XWayland/Wine)
    //     if ((iconName === "" || iconName === "application-x-executable") && windowData?.title) {
    //         // Simple heuristic: use the first word of the title as a fallback
    //         iconName = windowData.title.split(' ')[0].toLowerCase();
    //     }

    //     // 3. Pass the corrected name into the original Quickshell function
